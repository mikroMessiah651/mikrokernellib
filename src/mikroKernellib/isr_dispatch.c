...
static inline void pf_handler(const registers_t* regs) {
    pf_info_t pf_info;
    ...
    // vm_handle_page_fault((uint64_t)pf_info.vaddr);
}
...
void isr_dispatch(const registers_t* regs) {
    ...
    case 2:
        vesa_nt_println("NMI  Non-Maskable Interrupt, vector: 0x02",
                        VESA_LAST_ROW, 0);
        while (1) {
            hlt;
        }
    ...
    case 5:
        vesa_nt_println("#BR  BOUND Range Exceeded, vector: 0x05",
                        VESA_LAST_ROW, 0);
        while (1) {
            hlt;
        }
    ...
    case 7:
        vesa_nt_println("#NM  Device Not Available, vector: 0x07",
                        VESA_LAST_ROW, 0);
        while (1) {
            hlt;
        }
    ...
    case 9:
        vesa_nt_println("Coprocessor Segment Overrun, vector: 0x09",
                        VESA_LAST_ROW, 0);
        while (1) {
            hlt;
        }
    ...
    case 11:
        vesa_nt_println("#NP  Segment Not Present, vector: 0x0B", VESA_LAST_ROW,
                        0);
        while (1) {
            hlt;
        }
    ...
    case 12:
        vesa_nt_println("#SS  Stack-Segment Fault, vector: 0x0C", VESA_LAST_ROW,
                        0);
        while (1) {
            hlt;
        }
    ...
    case 13:
        vesa_nt_println("#GP  General Protection fault, vector: 0x0D",
                        VESA_LAST_ROW, 0);
        vesa_print_virt_addr(regs->error_code, VESA_LAST_ROW - 2, 0);
        while (1) {
            hlt;
        }
        break;
    ...
    case 19:
        vesa_nt_println("#XM  SIMD Floating-Point Exception, vector: 0x13",
                        VESA_LAST_ROW, 0);
        while (1) {
            hlt;
        }
    ...
    case 20:
        vesa_nt_println("#VE  Virtualization Exception, vector: 0x14",
                        VESA_LAST_ROW, 0);
        while (1) {
            hlt;
        }
    ...
    case 21:
        vesa_nt_println("#CP  Control Protection, vector: 0x15", VESA_LAST_ROW,
                        0);
        while (1) {
            hlt;
        }
    ...
    case 28:
        vesa_nt_println("#HV  Hypervisor Injection Exception, vector: 0x1C",
                        VESA_LAST_ROW, 0);
        while (1) {
            hlt;
        }
    ...
    case 29:
        vesa_nt_println("#VC  VMM Communication Exception, vector: 0x1D",
                        VESA_LAST_ROW, 0);
        while (1) {
            hlt;
        }
    ...
    case 30:
        vesa_nt_println("#SX  Security Exception, vector: 0x1E", VESA_LAST_ROW,
                        0);
        while (1) {
            hlt;
        }
